...
Eigen::Matrix3d best_fit_transform(const Eigen::MatrixXd &A, const Eigen::MatrixXd &B){
    // Because T matrix is 3 x 3 for Rotation matrix and translation matrix
    Eigen::Matrix3d T = Eigen::MatrixXd::Identity(3, 3);
    Eigen::Vector3d centroid_A(0, 0);
    Eigen::Vector3d centroid_B(0, 0);
    Eigen::MatrixXd AA = A;
    Eigen::MatrixXd BB = B;
...
ICP_OUT icp(const Eigen::MatrixXd &A, const Eigen::MatrixXd &B, int max_iterations, int tolerance){
    ...
    for(int i=0; i<max_iterations; i++){
        neighbor = nearest_neighbot(src2d.transpose(), B);
        for(int j=0; j<row; j++){
            src2d.block<2, 1>(0, j) = src.block<2, 1>(0, neighbor.indices[j]);
        }
